...
import { createCarAction, type CreateCarActionResult } from "@/features/cars/server/create-car-action";
...
const clientSchema = z
  ...
  .refine((values) => values.condition !== "OTHER" || values.conditionOther.trim(), {
    path: ["conditionOther"],
...
export function BuyCarForm({
  ...
  function fieldError(name: keyof FormValues): string | undefined {
    return errors[name]?.message ?? actionResult?.ok === false
    ...
    <form onSubmit={handleSubmit(submit)} className="space-y-6" noValidate>
      {!servicesReady && (
        <div className="flex gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
          ...
          <div>
            <p className="font-medium">Connect Neon and Clerk to save purchases</p>
            <p className="mt-1 text-amber-900/80">
              The complete form is available now, but saving stays disabled until the required
        ...
        <div role="alert" className="flex gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
          ...
          <div>
            ...
            {actionResult.duplicateVin && (
              <p className="mt-1">
                Match: {actionResult.duplicateVin.carNumber} · {actionResult.duplicateVin.carLabel}
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-6">
          <FormSection title="Basic information" description="Identify the vehicle and purchase date.">
            ...
            <Field label="Purchase date *" error={fieldError("purchaseDate")}>
              <Input type="date" {...register("purchaseDate")} aria-invalid={Boolean(fieldError("purchaseDate"))} />
            </Field>
            <Field label="Brand *" error={fieldError("brand")}>
              <Input list="car-brands" placeholder="Toyota" {...register("brand")} />
              <datalist id="car-brands">
                {commonBrands.map((brand) => <option key={brand} value={brand} />)}
              </datalist>
            </Field>
            ...
            <Field label="Year" error={fieldError("year")}>
              <Input inputMode="numeric" maxLength={4} placeholder="2018" {...register("year")} />
            </Field>
            ...
            {values.condition === "OTHER" && (
              <Field label="Other condition *" error={fieldError("conditionOther")} className="sm:col-span-2">
                <Input placeholder="Describe the vehicle condition" {...register("conditionOther")} />
              </Field>
            )}
          </FormSection>

          <FormSection title="Purchase" description="Record the initial investment only.">
            <Field label="Purchase price (AED) *" error={fieldError("purchasePrice")}>
              <Input inputMode="decimal" placeholder="8000.00" {...register("purchasePrice")} />
            </Field>
            ...
            <Field label="VIN / Chassis" error={fieldError("vinChassis")} className="sm:col-span-2">
              <Input placeholder="Searchable vehicle identifier" {...register("vinChassis")} />
            </Field>
            {actionResult?.ok === false && actionResult.duplicateVin && (
              <label className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm sm:col-span-2">
                <input type="checkbox" className="mt-1" {...register("confirmDuplicateVin")} />
                <span><strong>Admin override:</strong> I reviewed the duplicate VIN warning and want to continue.</span>
              </label>
            )}
            <label className="flex items-start gap-3 text-sm sm:col-span-2">
              <input type="checkbox" className="mt-1" {...register("allowFutureDate")} />
              <span>Admin override for a future purchase date</span>
            </label>
          </FormSection>

          <FormSection title="Seller" description="Select the actual owner or person selling this vehicle." single>
            <Field label="Seller *" error={fieldError("sellerId")}>
              <ReferencePicker
                options={sellers}
                value={values.sellerId}
                onChange={(id) => setValue("sellerId", id, { shouldValidate: true })}
          ...
          <FormSection title="Source" description="Track how this purchase reached the business.">
            <Field label="Source type *" error={fieldError("sourceType")}>
              ...
              >
                {sourceTypeValues.map((value) => <option key={value} value={value}>{sourceTypeLabels[value]}</option>)}
              </NativeSelect>
            </Field>
            <Field
          ...
          <FormSection title="Photos" description="Upload vehicle images and choose the main photo." single>
            <PhotoPicker
              files={photos}
          ...
          <FormSection title="Notes" description="Operational information for your team." single>
            <Field label="Notes" error={fieldError("notes")}>
              <Textarea rows={5} maxLength={2000} placeholder="Optional operational notes" {...register("notes")} />
            </Field>
          </FormSection>
        </div>
        ...
        <aside className="xl:sticky xl:top-24 xl:self-start">
          <Card className="shadow-sm">
            <CardHeader>
              ...
              <CardDescription>Car expenses start at zero and are added later.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="flex items-center gap-3 rounded-lg bg-muted p-3">
                ...
                <div className="min-w-0">
                  ...
                  <Badge variant="outline" className="mt-1">New purchase</Badge>
                </div>
              </div>
              <SummaryLine label="Purchase price" value={formatAed(values.purchasePrice)} />
              <SummaryLine label="Car expenses" value="AED 0.00" />
              <div className="border-t pt-4">
                <SummaryLine label="Initial investment" value={formatAed(values.purchasePrice)} strong />
              </div>
              <Button type="submit" size="lg" className="w-full" disabled={isPending || !servicesReady}>
                <Save className="size-4" />
                {isPending ? "Saving car…" : "Save Car"}
              </Button>
              <p className="text-center text-xs leading-5 text-muted-foreground">
                Saving creates the car and its matching money-out purchase transaction.
              </p>
            </CardContent>
          </Card>
        </aside>
...
function FormSection({
  ...
  return (
    <Card>
      ...
      <CardContent className={single ? "space-y-4" : "grid gap-5 sm:grid-cols-2"}>{children}</CardContent>
    </Card>
  );
}
...
function SummaryLine({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className={strong ? "flex justify-between gap-4 font-semibold" : "flex justify-between gap-4 text-sm"}>
...
const commonBrands = ["Toyota", "Nissan", "BMW", "Mercedes-Benz", "Honda", "Ford", "Hyundai", "Kia", "Lexus"];
